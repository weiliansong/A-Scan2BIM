...
class EdgeEnum(nn.Module):
    def __init__(
        ...
        self.output_fc = MLP(
            input_dim=hidden_dim, hidden_dim=hidden_dim // 2, output_dim=2, num_layers=2
        )
...
class EdgeTransformer(nn.Module):
    def __init__(
        ...
        self.input_fc_hb = MLP(
            input_dim=2 * d_model, hidden_dim=d_model, output_dim=d_model, num_layers=2
        )
        self.input_fc_rel = MLP(
            input_dim=2 * d_model, hidden_dim=d_model, output_dim=d_model, num_layers=2
        )

        self.output_fc_1 = MLP(
            input_dim=d_model, hidden_dim=d_model // 2, output_dim=2, num_layers=2
        )
        self.output_fc_2 = MLP(
            input_dim=d_model, hidden_dim=d_model // 2, output_dim=2, num_layers=2
        )
        self.output_fc_3 = MLP(
            input_dim=d_model, hidden_dim=d_model // 2, output_dim=2, num_layers=2
        )
        self.output_fc_4 = MLP(
            input_dim=d_model, hidden_dim=d_model // 2, output_dim=48, num_layers=2
        )
        self.output_fc_5 = MLP(
            input_dim=d_model, hidden_dim=d_model // 2, output_dim=48, num_layers=2
        )
    ...
    def forward(
        ...
        # to simplify attention on many reference points
        assert (abs(valid_ratios - 1) < 1e-6).all()
        ...
        # relational decoder
        # hs_per_edge, _ = self.per_edge_decoder(
        #     tgt,
        #     reference_points,
        #     memory,
        #     spatial_shapes,
        ...
        #     mask_flatten,
        # )
        hs_per_edge, per_ref_in, per_ref_out = self.per_edge_decoder(
            tgt=tgt,
        ...
        if get_hs:
            return hs_per_edge, logits_per_edge
        ...
        if skip_filtering:
            filtered_hs = hs_per_edge
            filtered_mask = key_padding_mask
            filtered_query = query_embed
            filtered_rp = reference_points
            filtered_coords = edge_coords
            filtered_labels = labels
            selected_ids = (
                torch.arange(hs_per_edge.shape[1]).unsqueeze(0).to(hs_per_edge.device)
            )
        elif do_inference:
            (
                filtered_hs,
                ...
                filtered_labels,
                selected_ids,
            ) = self.candidate_filtering_inference(
                logits=logits_per_edge,
                hs=hs_per_edge,
                query=query_embed,
                rp=reference_points,
                coords=edge_coords,
        ...
        if mask_gt:
            filtered_gt_values = self.generate_gt_masking(
                filtered_labels, filtered_mask
            )
        else:
            assert (labels == 2).all()
        ...
        hs, hb_ref_in, hb_ref_out = self.relational_decoder(
            tgt=hybrid_prim_hs,
            reference_points=filtered_rp,
            src=memory,
            src_spatial_shapes=spatial_shapes,
            src_level_start_index=level_start_index,
            src_valid_ratios=valid_ratios,
            query_pos=filtered_query,
            edge_coords=filtered_coords,
            image_size=image_size,
            src_padding_mask=mask_flatten,
            key_padding_mask=filtered_mask,
            get_image_feat=True,
        )

        logits_edge_hb = self.output_fc_2(hs).permute(0, 2, 1)
        logits_width_hb = self.output_fc_4(hs).permute(0, 2, 1)

        # relational decoder without image feature
        rel_prim_hs = self.input_fc_rel(torch.cat([filtered_query, gt_info], dim=-1))

        hs_rel, _, _ = self.relational_decoder(
            tgt=rel_prim_hs,
            reference_points=filtered_rp,
            src=memory,
            src_spatial_shapes=spatial_shapes,
        ...
        logits_edge_rel = self.output_fc_3(hs_rel).permute(0, 2, 1)
        logits_width_rel = self.output_fc_5(hs_rel).permute(0, 2, 1)

        ref_dict = {
            "per_ref_in": per_ref_in,
            "per_ref_out": per_ref_out,
            "hb_ref_in": hb_ref_in,
            "hb_ref_out": hb_ref_out,
        }

        if get_embed:
            assert not filtered_mask.any()
            assert (filtered_gt_values == filtered_labels).all()
            return hs, hs_rel
        else:
            return (
                logits_per_edge,
    ...
    def candidate_filtering(
        ...
        for b_i in range(B):
            ...
            filtered_coords.append(coords[b_i][ids])
        ...
        filtered_coords = torch.stack(filtered_coords, dim=0)
    ...
    @staticmethod
    def candidate_filtering_inference(
        logits,
        hs,
        query,
        rp,
        ...
        # first keep all the condition edges
        keep_mask = labels != 2

        # then for the remainder, keep the top-k
        preds = logits.detach().softmax(1)[:, 1, :]  # BxL
        preds[key_padding_mask == True] = -1  # ignore the masking parts
        remainder_preds = preds[~keep_mask]
        sorted_ids = torch.argsort(remainder_preds, descending=True)

        offset = keep_mask.sum()
        for keep_id in sorted_ids[:max_candidates]:
            keep_mask[0, offset + keep_id] = True

        # filter
        filtered_hs = hs[keep_mask].unsqueeze(0)
        filtered_mask = key_padding_mask[keep_mask].unsqueeze(0)
        filtered_query = query[keep_mask].unsqueeze(0)
        filtered_rp = rp[keep_mask].unsqueeze(0)
        filtered_coords = coords[keep_mask].unsqueeze(0)
        filtered_labels = labels[keep_mask].unsqueeze(0)

        all_ids = torch.arange(L).unsqueeze(0)
        selected_ids = all_ids[keep_mask].unsqueeze(0)

        assert (filtered_labels != 2).sum() == (labels != 2).sum()
    ...
    def generate_gt_masking(labels, mask):
        ...
        for b_i in range(bs):
            ...
            gt_values[
                b_i, pred_idx[0]
            ] = 2  # use 2 to represent unknown value, need to predict
        return gt_values
