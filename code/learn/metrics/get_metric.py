...
class Metric:
    def calc(self, gt_data, conv_data, thresh=8.0, iou_thresh=0.7):
        ...
        per_corner_score = {
            ...
            "precision": per_sample_corner_tp
            / (per_sample_corner_tp + per_sample_corner_fp + 1e-8),
        }
        ...
        for l, e_det in enumerate(conv_data["edges"]):
            ...
            for k, e_annot in enumerate(edge_corner_annots):
                c3, c4 = e_annot
                if ((c1_prime == c3) and (c2_prime == c4)) or (
                    (c1_prime == c4) and (c2_prime == c3)
                ):
                    is_hit = True
                    match_pred_ids.add(l)
                    break
        ...
        per_edge_score = {
            ...
            "precision": per_sample_edge_tp
            / (per_sample_edge_tp + per_sample_edge_fp + 1e-8),
        }
        ...
        for i, r_det in enumerate(all_conv_masks):
            ...
            for k, r_gt in enumerate(all_gt_masks):
                iou = np.logical_and(r_gt, r_det).sum() / float(
                    np.logical_or(r_gt, r_det).sum()
                )
                if iou > near_gt[1]:
                    near_gt = [k, iou, r_gt]
        ...
        # }
        per_region_score = {"recall": 1.0, "precision": 1.0}
        ...
        return {
            ...
            "matched_pred_edge_ids": list(match_pred_ids),
            "false_pred_edge_ids": false_edge_ids,
        }

    def calc_corner(self, gt_data, conv_data, thresh=8.0, iou_thresh=0.7):
        ### compute corners precision/recall
        gts = gt_data["corners"]
        dets = conv_data["corners"]
...
def compute_metrics(gt_data, pred_data, thresh=8.0):
    metric = Metric()
    score = metric.calc(gt_data, pred_data, thresh=thresh)
    return score
...
if __name__ == "__main__":
    ...
    for file_name in os.listdir(base_path):
        ...
        gt_data = np.load(
            os.path.join(gt_datapath, file_name + ".npy"), allow_pickle=True
        ).tolist()
    ...
    print(
        "corners - precision: %.3f recall: %.3f f_score: %.3f"
    ...
    f_score = 2.0 * precision * recall / (recall + precision + 1e-8)
    print(
        "edges - precision: %.3f recall: %.3f f_score: %.3f"
        % (precision, recall, f_score)
    )
    f.write(
        "edges - precision: %.3f recall: %.3f f_score: %.3f\n"
    ...
    f_score = 2.0 * precision * recall / (recall + precision + 1e-8)
    print(
        "regions - precision: %.3f recall: %.3f f_score: %.3f"
        % (precision, recall, f_score)
    )
    f.write(
        "regions - precision: %.3f recall: %.3f f_score: %.3f\n"
