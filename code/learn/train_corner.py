...
from datasets.building_corners import (
...
def train_one_epoch(
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        loss_dict = {"loss_c_s1": corner_loss, "corner_recall": corner_recall}
...
def run_model(
    ...
    corner_loss_s1, corner_recall = corner_criterion(
        preds_s1, pixel_labels, gauss_labels, epoch
    )

    # NOTE only training corner model for now
    return None, corner_loss_s1, corner_recall, None, None, None, 0, 0, 0, 0, 0, 0

    idx = 4
    _img = np.max(image[idx].permute(1, 2, 0).cpu().numpy(), axis=2)
    _lbl = gauss_labels[idx].detach().cpu().numpy()
    _pred = preds_s1[idx].detach().cpu().numpy()

    fig, [ax1, ax2] = plt.subplots(ncols=2)

    ax1.imshow(_img, cmap="gray")
    ax1.imshow(_lbl, cmap="hot", alpha=0.5)

    ax2.imshow(_img, cmap="gray")
    ax2.imshow(_pred, cmap="hot", alpha=0.5)
...
def main(args):
    ...
    # for blah in tqdm(train_dataset):
    #     pass
    ...
    all_params = corner_params + backbone_params
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # NOTE changed to corner recall for now
